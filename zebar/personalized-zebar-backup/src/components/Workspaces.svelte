...
{#if glazewm}
  <div class="flex flex-row items-center gap-2">
    {#each glazewm.currentWorkspaces as workspace, i}
      <button
        class="workspace-number text-zb-ws-{i} {workspace.hasFocus ? 'workspace-focused' : 'workspace-unfocused'} bg-blend-30 border border-zb-border rounded-base shadow-button hover:translate-x-boxShadowX hover:translate-y-boxShadowY hover:shadow-none transition-all duration-75"
        onclick={() =>
